<table class="table user-table table-hover align-items-center">
    <thead>
        <tr>
            ...
            <th class="border-bottom">vendore name</th>
    ...
    <tbody>
        @forelse ($stores as $store)
            <tr>
                ...
                <td>
                    <div class="d-block">
                        <span class="fw-bold">{{ $store->store_name ?? 'N/A' }}</span>
                    </div>
                </td>
                <td>
                    <span class="fw-normal">
                        @if ($store->vendor)
                            {{$store->vendor->first_name ?? '-' }}
                        @else
                            -
                        @endif
                    </span>
                ...
                <td>
                    <span class="fw-normal {{ $store->location }}">
                        {{ $store->location }}
                    </span>
                </td>
                <td>
                        ...
                        <div class="dropdown-menu dashboard-dropdown dropdown-menu-start mt-2 py-1">
                            ...
                            @can('user-delete')
                                <a class="dropdown-item text-danger d-flex align-items-center gap-1" href="#"
                                    onclick="confirmDelete(event, '{{ route('admin.store.destroy', jsencode_userdata($store->id)) }}', 'Are you sure you want to delete this category?')">
...
<div class="card-footer px-3 border-0 d-flex flex-column flex-lg-row align-items-center justify-content-between">
    ...
    <div class="fw-normal small mt-4 mt-lg-0">Showing
        <b>{{ $stores->lastItem() - $stores->firstItem() + 1 }}</b> out of <b>{{ $stores->total() }}</b>
        entries
    </div>
</div>
