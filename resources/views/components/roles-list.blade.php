    ...
    <tbody>
        @forelse ($roles as $role)
            <tr>
                ...
                <td>
                    <span class="fw-bold">{{ $role->name ?? 'N/A' }}</span>
                </td>
                <td>
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu dashboard-dropdown dropdown-menu-start mt-2 py-1 ml-3">
                            @can('user-edit')
                                <a class="dropdown-item d-flex align-items-center gap-1"
                                    href="{{ route('admin.role.createOrUpdate', ['role' => $role->id]) }}">
                            ...
                            @can('user-delete')
                                <a class="dropdown-item text-danger d-flex align-items-center gap-1" href="#"
                                    onclick="confirmDelete(event, '{{ route('admin.role.destroy', ['role' => $role->id]) }}', 'Are you sure you want to delete this role?')">
...
<div class="card-footer px-3 border-0 d-flex flex-column flex-lg-row align-items-center justify-content-between">
    ...
    <div class="fw-normal small mt-4 mt-lg-0">Showing <b>{{ ($roles->lastItem() - $roles->firstItem()) + 1 }}</b> out of <b>{{ $roles->total() }}</b> entries</div>
</div>